...
Given its lucrative advantages, software reuse is definitely beneficial for many software systems. However, according to \citet{krueger_software_1992}, software reuse can be problematic and even disadvantageous. Learning to use a specific piece of reusable software often takes considerable effort. Moreover, finding suitable code fragments may also prove to be a challenge. For uncomplicated software systems and especially reusable components, it may not be worth the effort. Therefore, developer needs to carefully consider all sides of reusing when building a software system. \fixme{This needs a lot more flesh.} 

\fixme{Advantages and disadvantages in general about reuse, not about different methods as those are described in the reuse methods chapter. Software reuse metrics research \citep{mohagheghi_empirical_2008,frakes_software_1996,selby_enabling_2005} probably has some good points. Also, \citet{johnson_frameworkscomponents+_1997} may have something.}

\subsection{Software Reuse Methods}

% Probably not needed as a separate subsection?
% \subsection{Dimensions for Software Reuse}

Different reuse methods excel at different areas. In order to analyze different methods, it is first needed to analyze reusing software in general. \citep{krueger_software_1992} separates making reusable software into four areas, or dimensions: abstraction, selection, specialization and integration. Different reuse methods use these dimensions to a varying degree. The dimensions are presented in more detail below.

~

\textbf{Abstraction} is the process of making a piece of software more generic, thus making it applicable to a wider range of software projects. Software reuse is almost always based on abstraction, but according to \citep{krueger_software_1992}, raising the abstraction level has proven to be difficult, thus making building reusable software a nontrivial process.\newline
